...
export default function DarkWebScan() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setResult("");
    setTerminalLogs(["Initializing scan...", `Editing: ${email}`]);
    ...
    const interval = setInterval(() => {
      if (step < logSteps.length) {
        setTerminalLogs((prevLogs) => [...prevLogs, logSteps[step]]);
        step++;
      } else {
        ...
        <form onSubmit={handleSubmit}>
          <input
            ...
            onChange={(e) => {
              ...
              setTerminalLogs([`Editing: ${e.target.value}`]); // Update terminal logs while typing
            }}
